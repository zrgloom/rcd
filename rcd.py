...
DESCRIPTION
-----------
rcd distributes arbitrary command sequences across multiple Cisco (or non-Cisco)
devices over SSH or Telnet. Before starting, prepare two files: a host file listing target devices and a command file listing the commands to distribute.
Lines beginning with '#' are treated as comments in both files.

\b
HOST FILE
---------
The first non-comment line is the parameter line — a comma-separated
list of column names.  Every following line supplies values for those
columns.  Column order does not matter.  If a value is omitted for a
column it can be supplied on the command line with a '-name value' pair;
values in the host file take precedence over command-line values.

\b
Mandatory columns:
\b
  ip      IP address of the target device.  Any octet may be a range:
          192.168.20-30.1-255 expands left-to-right, last octet fastest.
  login   Login name used during SSH/Telnet authentication.
  pass    Password (see Password management below).
  proto   Transport protocol: ssh or telnet.
  cfile   Path to the command file to execute on this device.
\b
User-defined columns:
  Any additional column name becomes a $variable usable in the command
  file.  Examples: enabpass, tftp, contexts, intname.
\b
Host file example:
\b
  #-- file: hosts ------------------------------------------
  ...
  #----------------------------------------------------------
\b
  The tftp value is omitted above; supply it on the command line:
\b
...
\b
  Plain text    topsecret
  File path     /etc/rcd/mypassword.txt    (first line of file is used)
  Keyring       keyring:<service>          (fetched via Python keyring CLI)
\b
The keyring form also supports TOTP/OTP tokens.  Use --otp so that rcd
waits out the remainder of the current 30-second window before retrying
with a fresh token after a failed login.  The default keyring backend is
keyring.backends.libsecret.Keyring (~/.local/share/keyrings/).
Install with: pip install keyring
\b
Enable passwords must be stored as plain text in the host file, or use
AAA authorization to enter privileged mode automatically.

\b
COMMAND FILE
------------
Lines alternate strictly between expect and send roles.  The first line
is an expect pattern, the second is a send line, the third is an expect
pattern, and so on.  Blank send lines press Enter (useful for accepting
default prompts).  Variable references ($colname, $1..$n) are substituted
before each line is used.

\b
  $1..$n are populated from regex capture groups of the most recent
  expect match.  Example: the expect pattern (\\w+)# stores the device
  hostname in $1, which can then be used in a send line such as
  'write net $tftp:$1'.

\b
Command file example (PIX/ASA backup):
\b
  #-- file: cisco.pix --------------------------------------
  .*>
  enable
...
\b
REGULAR EXPRESSIONS (expect lines)
-----------------------------------
\b
  ...
  *       Zero or more of the previous item.
  ...
  ( )     Groups a subpattern (capture group → $1..$n).
  |       Alternation.
  ...
  \\s      Whitespace.
\b
  This is a shortened list; consult Python 're' module documentation
  for the full syntax.

\b
LOOP / ENDLOOP
--------------
LOOP iterates over a pipe-separated list variable without re-logging in.
The loop body follows the same expect/send alternation as the rest of the
file.  The first expect of each iteration catches the response from the
previous iteration's last send (or the send that immediately preceded the
LOOP line for the first iteration).
\b
  After ENDLOOP always add an explicit expect to catch the last
  iteration's final response.  Do NOT use blank lines around LOOP —
  blank lines become empty expects that match immediately and swallow
  output without displaying it.  Loops may be nested.
\b
Loop example (ASA security contexts):
\b
  .*>
  enable
  [Pp]assword:
...
DYNAMIC VARIABLES
-----------------
Any variable can be populated at runtime by querying the device itself.
Use the 'cmd:' prefix in the host file value.  The variable is resolved
silently the first time it is referenced and then cached.  It works in
send lines, expect patterns and LOOP list variables.
\b
Syntax:
\b
  cmd:device_cmd1;pattern1;device_cmd2;pattern2;... [ | os_cmd1 | os_cmd2 ]
\b
  Semicolons separate command/pattern pairs.  Each command is sent to
  the device; the pattern (Python regex) marks the end of that command's
  output.  Output from all pairs is concatenated.  The first ' | '
  (space-pipe-space) separates the device part from an optional OS
  pipeline passed to /bin/sh, so awk, grep, sed and further '|' chains
  all work normally.  Output lines become the variable value; for LOOP
  variables items are joined with '|'.
\b
  Shell quoting rules apply to the OS pipeline.  Use single quotes
  around patterns containing backslashes (grep '\\w\\+', not grep \\w\\+
  — the shell strips unquoted backslashes before grep sees them).
\b
...
\b
  ...
  contexts = cmd:enable;[Pp]assword:;$enabpass;.*#;show context;.*# | awk '{print $1}'

\b
USAGE NOTICE
------------
Always test your command file on a lab device before running it in
production.  A mistake in the command file can leave devices in an
unreachable state.
"""
